...
object FuncLib {
  ...
  abstract class Funct1[A, F] { self =>
    final def `then`[U](fn: A => U): Funct1[U, F] = new Funct1[U, F] {
  ...
  abstract class Funct2[A, B, F] { self =>
    ...
    final def `with`[T](t: T): Funct3[A, B, T, F] = new Funct3[A, B, T, F] {
      def apply(f: F): (A, B, T) = self.apply(f) match { case (a, b) => (a, b, t) }
    }
    final def and[C, F_](that: Funct1[C, F_]): Funct3[A, B, C, (F, F_)] = new Funct3[A, B, C, (F, F_)] {
      def apply(ff: (F, F_)): (A, B, C) = ff match {
        case (f, f_) => self.apply(f) match { case (a, b) => (a, b, that.apply(f_)) }
      }
    }
  ...
  abstract class Funct3[A, B, C, F] { self =>
    def `then`[U](fn: (A, B, C) => U): Funct1[U, F] = new Funct1[U, F] {
      override def apply(f: F): U = self.apply(f) match { case (a, b, c) => fn(a, b, c) }
    }
    def apply(f: F): (A, B, C)
  }
